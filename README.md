...
settingsPage.CanvasSize=UDim2.new(0,0,0,680)
settingsPage.ScrollBarThickness=5
settingsPage.ScrollBarImageColor3=purple
settingsPage.ScrollBarImageTransparency=.15
settingsPage.ScrollingDirection=Enum.ScrollingDirection.Y
settingsPage.ElasticBehavior=Enum.ElasticBehavior.Always
...
local function makeToggle(parent,textValue,y,desc)
	...
	icon.Text=textValue=="Fly" and "✈" or textValue=="Speed" and "ϟ" or textValue=="Jump" and "↑" or textValue=="ESP" and "◉" or textValue=="FPS" and "▣" or "♟"
	icon.Parent=card
	corner(icon,9)
	local titleLabel=Instance.new("TextLabel")
	titleLabel.Size=UDim2.new(1,-100,0,24)
	titleLabel.Position=UDim2.new(0,64,0,11)
...
local function makeSlider(parent,titleValue,y,initial,minValue,maxValue,onChanged)
	local card=Instance.new("Frame")
	...
	icon.TextSize=21
	icon.Font=Enum.Font.GothamBold
	icon.Text=titleValue=="Fly" and "✈" or titleValue=="Speed" and "ϟ" or "↑"
	icon.Parent=card
	corner(icon,9)
	...
	desc.Text="Enable or disable "..string.lower(titleValue)
	...
	label.Text=titleValue=="Speed" and "Walk Speed" or titleValue=="Jump" and "Jump Power" or "Fly Speed"
	...
	local function setValue(v)
		v=math.clamp(math.floor(v+.5),minValue,maxValue)
	...
	knobSlider.MouseButton1Down:Connect(function()
		draggingSlider=true
	end)
...
moveTitle.Size=UDim2.new(1,0,0,24)
moveTitle.Position=UDim2.new(0,0,0,72)
moveTitle.BackgroundTransparency=1
moveTitle.Text="MOVE"
moveTitle.TextColor3=purple
moveTitle.TextSize=13
moveTitle.Font=Enum.Font.GothamBold
moveTitle.TextXAlignment=Enum.TextXAlignment.Left
moveTitle.Parent=settingsPage

local flyCard,flyBox,setFly=makeSlider(settingsPage,"Fly",100,flySpeed,1,300,function(v)
	flySpeed=v
end)

local speedCard,speedBox,setSpeed=makeSlider(settingsPage,"Speed",196,walkSpeed,1,300,function(v)
	walkSpeed=v
	if speedOn then
...
local jumpCard,jumpBox,setJump=makeSlider(settingsPage,"Jump",292,jumpPower,1,300,function(v)
	jumpPower=v
	if jumpOn then
		local char=player.Character
		local humanoid=char and char:FindFirstChildOfClass("Humanoid")
...
local noclipCard,noclipToggle,setNoClip=makeToggle(settingsPage,"NoClip",388,"Enable or disable noclip")
noclipCard.Size=UDim2.new(1,0,0,64)
noclipCard.Position=UDim2.new(0,0,0,388)

local showTitle=Instance.new("TextLabel")
showTitle.Size=UDim2.new(1,0,0,24)
showTitle.Position=UDim2.new(0,0,0,460)
showTitle.BackgroundTransparency=1
showTitle.Text="SHOW"
showTitle.TextColor3=purple
showTitle.TextSize=13
showTitle.Font=Enum.Font.GothamBold
showTitle.TextXAlignment=Enum.TextXAlignment.Left
showTitle.Parent=settingsPage

local espCard,espToggle,setESP=makeToggle(settingsPage,"ESP",490,"Show players through walls")
espCard.Size=UDim2.new(.5,-5,0,64)
espCard.Position=UDim2.new(0,0,0,490)

local fpsCard,fpsToggle,setFPS=makeToggle(settingsPage,"FPS",490,"Display current FPS")
fpsCard.Size=UDim2.new(.5,-5,0,64)
fpsCard.Position=UDim2.new(.5,5,0,490)

local mapTitle=Instance.new("TextLabel")
mapTitle.Size=UDim2.new(1,0,0,24)
mapTitle.Position=UDim2.new(0,0,0,562)
mapTitle.BackgroundTransparency=1
mapTitle.Text="MAP"
mapTitle.TextColor3=purple
mapTitle.TextSize=13
mapTitle.Font=Enum.Font.GothamBold
mapTitle.TextXAlignment=Enum.TextXAlignment.Left
mapTitle.Parent=settingsPage

local fixLagCard,fixLagToggle,setFixLag=makeToggle(settingsPage,"Fix Lag",592,"Reduce client-side visual effects")
fixLagCard.Size=UDim2.new(1,0,0,64)
fixLagCard.Position=UDim2.new(0,0,0,592)

local fpsLabel=Instance.new("TextLabel")
fpsLabel.Size=UDim2.new(0,100,0,30)
fpsLabel.Position=UDim2.new(1,-115,0,15)
fpsLabel.BackgroundColor3=panel2
fpsLabel.BackgroundTransparency=.05
fpsLabel.Text="FPS: --"
...
local DISCORD_AVATAR_ASSET="rbxassetid://YOUR_DISCORD_AVATAR_ASSET_ID"
...
fixLagToggle.MouseButton1Click:Connect(function()
...
connection=run.RenderStepped:Connect(function()
	...
	if now-fpsTime>=1 then
		if fpsOn then
			fpsLabel.Text="FPS: "..math.floor(fpsFrames/(now-fpsTime))
		end
		fpsFrames=0
		fpsTime=now
	end
